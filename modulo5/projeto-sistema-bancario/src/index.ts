      ...
      if (Number(day) < dayBirth) {
        ...
        for (let client of clients) {
          if (client.cpf === req.body.cpf) {
            throw new Error(
              "The provided CPF already exists in another registration. Please check your documents."
            );
          }
        }
...
app.get("/myaccount", (req, res) => {
  ...
  try {
    res
      .status(200)
      .send(
        `${clients[indexBalance].name}, your balance is: ${clients[indexBalance].balance} dollars.`
      );
  } catch (error) {
...
app.put("/myaccount", (req, res) => {
  ...
  const indexDeposit = clients.findIndex(
    (client) => client.name === name && client.cpf === cpf
  );
  ...
  try {
    if (indexDeposit != -1) {
      ...
      res
        .status(200)
        .send(
          `${clients[indexDeposit].name}, you have deposited ${cash} dollars. Your current  ${clients[indexDeposit].balance}`
        );
    } else {
...
app.post("/myaccount/bill", (req, res) => {
  ...
  try {
    ...
    throw new Error(
      "Something wrong happened. Check the informations provided or call your account manager."
    );
  }
});
