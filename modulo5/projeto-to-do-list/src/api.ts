...
const erros: { [key: string]: { status: number; message: string } } = {
  MISSING_PARAMETERS: {
    status: 422,
    message: "Informações inconsistentes; tente novamente.",
  },
  NOT_FOUND: {
    status: 404,
  ...
  try {
    ...
    switch (error.message) {
      case erros.MISSING_PARAMETERS.message:
        res
          .status(erros.MISSING_PARAMETERS.status)
          .send(erros.MISSING_PARAMETERS.message);
        break;
      default:
...
export const readTask = async (req: Request, res: Response) => {
  try {
    ...
    const task = await connection("userTask")
...
export const createTask = async (req: Request, res: Response) => {
  try {
    ...
    res.status(201).send("Nova tarefa adicionada!");
  } catch (error: any) {
    switch (error.message) {
      case erros.MISSING_PARAMETERS.message:
        res
